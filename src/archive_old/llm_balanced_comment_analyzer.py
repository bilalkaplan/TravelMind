...
INPUT_PATH = Path("data/processed/balanced_comment_eval_set.csv")
OUTPUT_PATH = Path("data/processed/balanced_llm_comment_predictions.csv")


SYSTEM_PROMPT = """
You are an evaluator for hotel comments.
...
- If the comment is sarcastic, ironic, mixed, vague, incomplete, or ambiguous, use "unclear" when necessary.
...
- Do not classify sarcastic praise as positive.
...
def initialize_foundry_client():
    ...
    config = Configuration(app_name="travelmind_rag_balanced_eval")
...
def main():
    if not INPUT_PATH.exists():
        ...
        print("python src\\create_balanced_comment_eval_set.py")
        return

    df = pd.read_csv(INPUT_PATH)

    if "room_comment" not in df.columns:
        print("balanced_comment_eval_set.csv içinde room_comment kolonu yok.")
        return
    ...
    limit_text = input("Kaç yorum analiz edilsin? Enter = hepsi: ").strip()
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            print("Manual label:", row.get("manual_sentiment_label", ""))
            print("LLM sentiment:", analysis["llm_sentiment"])
            print("Manual sarcasm:", row.get("sarcasm_or_irony", ""))
            print("LLM sarcasm:", analysis["llm_sarcasm_or_irony"])
        ...
        print("\n" + "=" * 80)
        print("Dengeli LLM yorum analizi tamamlandı.")
